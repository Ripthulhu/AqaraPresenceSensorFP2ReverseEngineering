...
- Live ESPHome sleep replay on 2026-06-07: the new `fp2_configure_sleep_mode` action reproduced the stock-app sleep setup's core UART writes. Confirmed sequence was `sleep_report_enable=TRUE`, blank `zone_detect_setting`, `work_mode=9`, blank `zone_detect_setting`, `location_report_enable=TRUE`, `sleep_zone_size=0x007800B4` (`120 x 180`), and `sleep_zone_mount_position=1`. The radar reported `work_mode=9`, ACKed `sleep_zone_size` as bytes `01 69 02 00 78 00 b4`, and ACKed `sleep_zone_mount_position`. No `sleep_data` was emitted during this replay when the live target count went to zero after switching modes; the stock capture still shows `sleep_data` beginning after the same setup when a target is present.
- The ESPHome fork now publishes command TX/ACK/retry/timeout events through `radar_debug`, which made the above replay visible through Home Assistant and native ESPHome logs. HA history coalesces fast text-sensor changes, so native `esphome logs` remains the best way to preserve full ordering.
